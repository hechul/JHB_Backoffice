...
| 2026-04-15 | 기능2 | 블로그 저장 정책을 기본 `블로그 1개 = ZIP 1개`로 정리하고, 용량 제한 초과 시 같은 블로그 묶음이 보이도록 `blog_01-1.zip`, `blog_01-2.zip` 형식으로 분할 저장 | 완료 |
| 2026-04-16 | 기능2 | Supabase `blog-media-zips` 버킷 실제 파일 제한이 50MB임을 확인하고, 워커 ZIP 분할 한도를 버킷 제한-헤드룸 기준(기본 45MB)으로 자동 보정 | 완료 |
