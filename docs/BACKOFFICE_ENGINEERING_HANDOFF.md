...
- 주차 필터는 `월요일 시작 + 그 주에 해당 달 날짜가 4일 이상 포함되면 그 달 주차로 인정`하는 표준 주차 규칙을 사용한다
- `일자별 실구매 판매량`은 현재 월 선택 기준으로 일자별, 주차 선택 시 해당 주(선택 월에 속한 날짜만) 일별, 전체 기간에서는 월별 판매량으로 렌더링한다
