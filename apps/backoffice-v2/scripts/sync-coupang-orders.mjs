...
async function fetchCoupangPurchasesByOrderIds(targetConfig, accountKey, orderIds) {
...
export async function runCoupangRevenueEnrichment(input) {
  ...
  const purchaseRows = input.targetConfig
    ? await fetchCoupangPurchasesByOrderIds(input.targetConfig, input.accountKey, orderIds)
    : []
  const purchaseLookup = buildPurchaseLookupByRevenueKey(purchaseRows)
...
async function main() {
  ...
  const summary = {
    ...
    revenueRecognitionTo: maxIsoDate(args.to, currentLocalIsoDate()),
    revenueHistoryRecordCount: 0,
  ...
  const revenueRunId = summary.scopeSummaries.find((scope) => normalizeString(scope.runId))?.runId || randomUUID()
