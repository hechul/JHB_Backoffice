...
| v6 | 2026-02-27 | 실제 구현 반영: 기술 스택 Nuxt 4, customers View 전환, 매칭 랭크 정의 재정렬, 이탈 기준 90일 조정, 비체험단 상품 추가, 업로드 롤백/중복방지/레거시 포맷 지원, filter_jobs 후속 이관, 후보 스코어링 알고리즘 명세 추가, 월 선택기 안정화(타임아웃/재시도/캐시/stale 가드), 헤더 새로고침 버튼 추가, 권한 로딩 폴백(무한 `확인중` 완화), 대시보드/알림/계정관리 1차 오픈 반영 |
...
| 매출 분석 > 대시보드 | Dashboard | KPI, 분포 차트, 이탈 경고 | ✅ 1차 완료 |
| 매출 분석 > 고객 분석 | Customers | 고객 목록 조회, 타겟 추출, 엑셀 다운로드 | ✅ 완료 |
| 매출 분석 > 실행 이력 | Logs | 필터링/수동분류 이력 조회 | ✅ 완료 |
| 상품 관리 > 상품 목록 | Products | 상품 마스터 CRUD | ✅ 완료 |
| 설정 > 계정 관리 | Users | 관리자 목록, 권한/상태 변경, 초대(후속) | ✅ 1차 완료 |

> `[v6 변경]` 각 메뉴의 구현 상태 컬럼 추가. 대시보드/계정관리는 1차 실데이터 전환 완료, 계정 초대 기능은 후속 구현.
...
3. **업로드 (Input)**:
   - 통합 엑셀 1개 (`.xlsx/.xls`) 업로드
   - 파일 내 주문 시트 + 체험단 시트 2개를 자동 탐지
...
> `[v6 변경]` 로그인 후 진입 지점을 "대시보드"에서 "홈 허브"로 변경 (실제 구현과 일치). 비동기 job_id/Polling 관련 상세는 §3.2.1에서 후속 구현으로 이관. 다운로드 포맷은 엑셀(`.xlsx`) 기준으로 명시.
...
Drag & Drop 기반 단일 파일 업로드. 통합 엑셀 파일 1개 내 2개 시트(주문 + 체험단) 입력을 기본으로 지원.

> `[v6 변경]` 업로드 입력 방식을 "통합 엑셀 1파일" 기준으로 고정.
...
- **위치**: [설정 > 계정 관리]
- **현재 상태**: 1차 오픈 완료 (사용자 목록 조회, 역할 변경, 활성/비활성 전환).
- **후속 기능**:
  - 이메일 입력 기반 신규 사용자 초대.
  - 계정 감사 로그(초대/권한 변경) 고도화.
  - 본인 비밀번호 및 프로필 변경.
- **권한 등급**:
...
| 고객 분석 | Code + DB 조회 | purchases 원본을 월/조건으로 조회 후 프론트에서 고객 단위 집계 |
| 통계/분석 | DB (SQL) | 매출 합계, 펫 타입 비율 등 집계 함수가 효율적 |
| 데이터 적재 | Code (클라이언트) | 엑셀 파싱, 정규화, UPSERT는 코드에서 수행 후 Supabase SDK로 적재 |

> `[v6 변경]` "서버"를 "클라이언트"로 변경. 현재 구현은 클라이언트 사이드(Nuxt composable)에서 필터링/집계/적재를 수행.
...
1. **고객 목록**: `purchases` 직접 조회 후 프론트 집계(고객별 그룹핑) + 페이지네이션(10건/페이지).
2. **펫 타입**: purchases × products(메타 조회) 기반 + 상품명 키워드 폴백 체인으로 결정.
3. **이탈 감지**: 최근 주문일 기준 90일 초과.
4. **다운로드**: 엑셀(`.xlsx`) 포맷으로 필터 조건 적용 결과 내보내기.

> `[v6 변경]` customers 테이블 UPDATE → purchases 직접 조회 + 프론트엔드 계산 방식으로 전환.
...
- **월 선택값 유지 원칙**: 사용자가 선택한 월은 0건 월이어도 유지하며, 자동으로 다른 월로 이동하지 않는다.
...
- **권한 표시 안정화**: 프로필 조회 실패 시 캐시/폴백으로 role 로딩을 종료해 헤더 권한 라벨이 `확인중`에 고정되지 않도록 한다.
...
**현재 구성**:
- **KPI 카드**: 전체 주문 / 실구매 / 재구매 고객(3회+) / 1회 구매 고객.
- **추이 차트**: 월별 실구매 추이(Line Chart, 월 선택 연동).
- **펫 타입 분포**: 실구매 고객 기준 DOG/CAT/BOTH 도넛 차트.
- **인기 상품 TOP5**: 실구매 주문 건수 기준.
- **성장 단계 분포**: 입문/성장/프리미엄 고객 수.
- **이탈 경고**: 90일 기준 이탈 위험군 상위 목록.
- **대시보드 → 고객 분석 딥링크**: KPI/버튼 클릭 시 고객 분석 페이지로 이동.
...
### 6.4 필터링 실행 (매출 분석 > 필터링)

- **결과 탭**: 체험단 매칭 / 실구매 건 / 미매칭 체험단 / 확인 필요.
- **탭 집계 기준**:
  - 체험단 매칭: `is_fake = true`
  - 실구매 건: `is_fake = false`
  - 확인 필요: `needs_review = true` (체험단/실구매 탭과 중복 노출 가능)
- **Rank 분포**: 매칭 유형별 건수 시각화.
- **수동 분류**: 개별 건 상세 패널에서 fake↔real 전환 + 저장 (커밋형 UX).
- **엑셀 다운로드**: 필터링 결과 정리 파일(`.xlsx`) 다운로드.
...
- **페이지네이션**: 10건/페이지.
- **엑셀 다운로드**: 현재 필터 조건 적용 결과(`.xlsx`).
- **URL 쿼리 동기화**: `q`, `petType`, `stage`, `churn`, `month`, `purchaseCount` 파라미터로 재진입/공유 일관성 유지.
...
**현재 상태**: ✅ 1차 오픈 (admin 전용).

**범위**: 사용자 목록 조회, role(admin/modifier/viewer) 변경, active/inactive 전환, 자기 계정/마지막 활성 admin 보호.
...
| **Phase 1** | 핵심 파이프라인 | 데이터 업로드(시트 감지/레거시 지원/배치 UPSERT/롤백/중복 방지), 5-Rank 필터링(키워드 매칭/스코어링/중복 제거/needs_review), 수동 분류(개별), 고객 분석(실데이터/필터/검색/엑셀), 실행 이력, 상품 Soft Delete/옵션 관리, 계정관리 1차(role/status) | ✅ 완료 |
| **Phase 2** | 대시보드 + 알림 | 대시보드 실데이터 전환(KPI/추이/분포/이탈 경고), 알림 센터(`notifications` 테이블 활성화), 운영 지표 확장/튜닝 | 🟡 진행중 |
| **Phase 3** | 관리 + 고도화 | 계정 관리(초대/감사로그), 일괄 수동 분류, 동시성 제어(Lock), 비동기 필터링(`filter_jobs` 활성화), 강제 매칭(미매칭 experience → purchase 수동 연결), 동적 이탈 예측(avg_purchase_cycle + 15일) | 📋 계획 |
...
3. 대시보드 월별 지표(실구매/체험단/이탈 위험) 실데이터 연동.
...
| §3.6 고객 분석 | `app/pages/customers.vue` | 필터, 검색, 엑셀(.xlsx), 딥링크 |
...
| 권한 라벨 로딩 | 미기재 | 프로필 조회 실패 시 캐시/폴백 적용 | 헤더 `확인중` 고정 현상 완화 |
